...
//
// Canonical encoding does not make an assessment reproducible — a
// provider-side model revision changes what it says, which is the whole
// reason the assessment is a separate artifact rather than part of the
// result document. It does mean that re-encoding an unchanged assessment
// produces unchanged bytes, so a diff between two artifacts shows what
// the model said differently and nothing else.
...
// able to publish it. It does carry real certnames — pseudonyms exist
// only in an inference request body — so it belongs wherever the JSON and
// HTML reports already go, and nowhere less protected than that.
