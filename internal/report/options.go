...
type Options struct {
	// ImpactNodes prints every certname an impact estimate returned instead
	// of the capped inline sample. A bounded estimate may hold up to its
	// configured result_limit certnames, a thousand by default, so the full
	// list is opt-in per estimate section rather than the default.
	ImpactNodes bool
}
...
// as a "+N more" tail, so the count is never hidden and only the names
// are.
