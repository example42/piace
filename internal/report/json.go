...
// JSON renders r as the versioned JSON report required by
// requirements.md 8.2, encoded with the in-tree canonical encoder per
// design.md section 9 ("JSON uses the in-tree canonical encoder").
//
// Encoding runs in two steps — encoding/json to bytes, then
...
// what makes design.md Property 1 hold end to end: struct field order,
// map insertion order, and how a number was originally spelled all
...
// declaration order; determinism is the property the design asks for, and
...
func JSON(r model.Result) ([]byte, error) {
	// requirements.md 9.3 requires the CLI to label the result a
	// **potential impact estimate**; the JSON report is one of the three
	// formats that obligation covers, and a bare `impact_estimates` key
	// carries no such label. The label and note are stamped onto a copy
	// here rather than upstream so the pipeline stays free of prose (see
	// model.Result's field comments and internal/impact's doc.go).
